module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'rankings') {
      ...
      if (!srcUrl) { res.status(400).json({ error: 'url required' }); return; }
      const imgResp = await fetch(srcUrl, { headers: { 'Referer': 'https://www.tiktok.com/', 'User-Agent': 'Mozilla/5.0' } });
      if (!imgResp.ok) { res.status(400).json({ error: 'Could not fetch image: ' + imgResp.status }); return; }
      const buf = await imgResp.arrayBuffer();
      const mime = imgResp.headers.get('content-type') || 'image/jpeg';
      const b64  = Buffer.from(buf).toString('base64');
      res.status(200).json({ ok: true, b64, mime });
      return;
    ...
    } else if (action === 'upload-image-b64') {
      const b64          = String(body.b64      || '').trim();
      const mimeType     = String(body.mimeType || 'image/jpeg').trim();
      const accessToken0 = String(body.accessToken || '').trim();
      if (!b64) { res.status(400).json({ error: 'b64 required' }); return; }
      const buf = Buffer.from(b64, 'base64');
      const ext = mimeType.includes('png') ? 'image.png' : mimeType.includes('gif') ? 'image.gif' : mimeType.includes('webp') ? 'image.webp' : 'image.jpg';
      const form = new FormData();
      form.append('file', new Blob([buf], { type: mimeType }), ext);
      const uploadResp = await fetch('https://four.meme/meme-api/v1/private/token/upload', {
        method:  'POST',
        headers: { ...(accessToken0 ? { 'meme-web-access': accessToken0 } : {}), 'origin': 'https://four.meme', 'referer': 'https://four.meme/' },
        body:    form,
      });
      const uploadData = await uploadResp.json().catch(() => ({}));
      res.status(200).json({ ok: true, data: uploadData });
      ...
      fourMemeBody = {
        ...
        desc:         body.description || body.name || '',
        ...
        rushMode:     false,
        onlyMPC:      false,
        feePlan:      body.taxInfo ? true : false,
        ...(body.taxInfo     ? { taxInfo:     body.taxInfo }     : {}),
